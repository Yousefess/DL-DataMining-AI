...
with tf.name_scope('placeholders'):
    X = tf.compat.v1.placeholder(shape=(None, n_features), dtype=tf.float32)
    y = tf.compat.v1.placeholder(shape=(None, n_output), dtype=tf.float32)
    keep_prob = tf.compat.v1.placeholder(tf.float32)

with tf.name_scope('hidden_layer'):
    W = tf.compat.v1.Variable(
        tf.compat.v1.random_normal((n_features, n_hidden)))
    b = tf.compat.v1.Variable(tf.zeros((n_hidden,)))
    X_hidden = tf.nn.relu(tf.matmul(X, W) + b)
    # Apply dropout
    X_hidden = tf.compat.v1.nn.dropout(X_hidden, keep_prob=keep_prob)

with tf.name_scope("output"):
    W = tf.compat.v1.Variable(tf.compat.v1.random_normal((n_hidden, n_output)))
    b = tf.compat.v1.Variable(tf.zeros((n_output,)))
...
train_writer = tf.compat.v1.summary.FileWriter(
    './nn_train', tf.get_default_graph())

with tf.name_scope("optim"):
    train_op = tf.compat.v1.train.AdamOptimizer(
        learning_rate=learning_rate).minimize(l)
